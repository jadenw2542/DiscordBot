...
class Welcome(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        welcome_embed = discord.Embed(title=f"Welcome to {member.guild.name}!", description=f"You are member #{member.guild.member_count}!", color=discord.Color.purple())
        welcome_embed.add_field(name="Welcome to to jungle", value=data[str(member.guild.id)]["Message"], inline=False)
        welcome_embed.set_image(url=data[str(member.guild.id)]["ImageUrl"])
        welcome_embed.set_footer(text="Hope you enjoy your stay!", icon_url=member.avatar)
    ...
    @commands.group(name="welcome", invoke_without_commands=True)
